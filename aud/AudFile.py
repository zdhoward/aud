...
class AudFile:
    ...
    def convertTo(
        ...
        if _extension.lower() == ".mp3":
            ...
            if _target_bitrate.lower() == "320k":
                command = "ffmpeg -i {0} -codec:a libmp3lame -b:a 320k {1}.mp3".format(
                    self.filepath,
                    os.path.join(
                        self.base, self.output_directory, self.name.split(".")[0]
                    ),
                )
            ## variable bitrate
            elif _target_bitrate.lower() == "0":
                command = "ffmpeg -i {0} -codec:a libmp3lame -q:a 0 {1}.mp3".format(
                    self.filepath,
                    os.path.join(
                        self.base, self.output_directory, self.name.split(".")[0]
                    ),
            ...
            if _target_bitdepth == 16:
                command = "ffmpeg -i {0} {1}.wav".format(
                    self.filepath,
                    os.path.join(
                        self.base, self.output_directory, self.name.split(".")[0]
                    ),
                ...
                command = "ffmpeg -i {0} -acodec s24le -ar {1} {2}.wav".format(
                    self.filepath,
                    _target_samplerate,
                    os.path.join(
                        self.base, self.output_directory, self.name.split(".")[0]
                ...
                command = "ffmpeg -i {0} -acodec s32le -ar {1} {2}.wav".format(
                    self.filepath,
                    _target_samplerate,
                    os.path.join(
                        self.base, self.output_directory, self.name.split(".")[0]
                ...
                command = "ffmpeg -i {0} -acodec s64le -ar {1} {2}.wav".format(
                    self.filepath,
                    _target_samplerate,
                    os.path.join(
                        self.base, self.output_directory, self.name.split(".")[0]
    ...
    def normalize(self, _type=None, _target=None):
        ...
        if _type == None and _target == None:
            command = "ffmpeg-normalize {0} -o {1}".format(
                self.filepath, os.path.join(self.base, self.output_directory, self.name)
            )
        elif _type != None and _target == None:
            command = "ffmpeg-normalize {0} -nt {1} -o {2}".format(
                self.filepath,
                _type,
                os.path.join(self.base, self.output_directory, self.name),
            )
        elif _type != None and _target != None:
            command = "ffmpeg-normalize {0} -nt {1} -t {2} -o {3}".format(
                self.filepath,
                _type,
                _target,
                os.path.join(self.base, self.output_directory, self.name),
